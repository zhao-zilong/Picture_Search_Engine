...
void read_cimage(char *name, CIMAGE *cim)
{
  ...
  /* djpeg envoie le r�sultat sur la sortie standard */
  /*-------------------------------------------------*/
  if (strstr(name,"http://") == name) {
    asprintf(&command,"/usr/local/bin/wget -O - \"%s\" -o /dev/null | /usr/local/bin/djpeg ",name);

  } else {
    asprintf(&command,"/usr/local/bin/djpeg \"%s\"",name);
  }
  /*-----------------------------------*/
